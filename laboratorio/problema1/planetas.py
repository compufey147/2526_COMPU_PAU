...
for i, p in enumerate(datos_planetas):
    ...
    r_ua = p[2] / AU_KM  # Si p[2] es negativo, r_ua es negativo
    e = p[3]

    r[i, 0] = abs(r_ua)  # La distancia siempre es positiva
    r[i, 1] = 0.0

    # El signo de r_ua decide si la velocidad va hacia arriba (+) o abajo (-)
    v[i, 1] = np.sign(r_ua) * np.sqrt((1.0 + e) / abs(r_ua))
